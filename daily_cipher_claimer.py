...
class DailyCipherClaimer(RequestsPayloadSample):
    def __init__(self):
        super().__init__()
        self.cipher_info = GCC.get_clicker_config().get("dailyCipher")
        self.__encoded_cipher_code = self.cipher_info.get("cipher")
        self.cipher_remain_seconds = self.cipher_info.get("remainSeconds")
        self.decoded_cipher_code = self.decode_security_code(self.__encoded_cipher_code)

    async def claim_daily_cipher(self) -> int:
        async with session.post("https://api.hamsterkombatgame.io/clicker/claim-daily-cipher",
                                                 headers=self._request_headers,
                                                 json={'cipher': self.decoded_cipher_code}
                                                 ).status as daily_cipher_status_code:
